...
function fmtDate(d: string | null | undefined, fmt: 'long' | 'short' = 'short'): string {
  if (!d) return '';
...
function dateRange(start: string | null, end: string | null): string {
  ...
  if (!end || end === start) return `${fmtDate(start)}, ${year}`;
  return `${fmtDate(start)} – ${fmtDate(end)}, ${year}`;
}

function daysInRange(start: string, end: string | null): Date[] {
  const result: Date[] = [];
...
export default function WeddingPublicPage() {
  ...
  if (loading) {
    return (
      <div style={{
        minHeight: '100vh', background: '#1a1510',
        display: 'flex', alignItems: 'center', justifyContent: 'center',
      }}>
        <p style={{ color: 'rgba(255,255,255,0.4)', fontFamily: 'Jost, sans-serif', letterSpacing: '0.2em', fontSize: '0.7rem', textTransform: 'uppercase' }}>
          Loading…
        </p>
      </div>
    );
  ...
  if (notFound || !wedding) {
    return (
      <div style={{
        ...
        <h1 className="font-display" style={{ fontSize: '2.5rem', fontWeight: 300, color: 'var(--charcoal)', marginBottom: '0.75rem' }}>
          Page not found
        </h1>
        <p className="font-sans-clean" style={{ color: 'var(--mid-gray)', fontSize: '0.85rem' }}>
          This wedding page doesn&apos;t exist or the link may have changed.
        </p>
      </div>
    );
  }

  const days = wedding.wedding_date ? daysInRange(wedding.wedding_date, wedding.wedding_end_date) : [];
  ...
  return (
    <div style={{ minHeight: '100vh', background: 'var(--cream)', fontFamily: 'Lora, Georgia, serif' }}>
      ...
      <div style={{
        ...
        <div style={{
          position: 'absolute', inset: 0,
          background: 'linear-gradient(180deg, rgba(10,8,5,0.3) 0%, rgba(10,8,5,0.6) 60%, rgba(10,8,5,0.85) 100%)',
        }} />
        ...
        <div style={{
          ...
          <h1 style={{
            ...
            <span style={{ display: 'block', fontStyle: 'italic', color: 'var(--blush)', fontSize: '0.65em' }}>
        ...
        {/* Scroll indicator */}
        {(wedding.welcome_message || days.length > 0) && (
          <div style={{
      ...
      {wedding.welcome_message && wedding.welcome_message !== 'EMPTY' && (
        <div style={{
          ...
          <p style={{
            marginTop: '2rem',
            fontFamily: 'Cormorant Garamond, Georgia, serif',
            fontSize: '1.1rem', color: 'var(--mid-gray)',
          }}>
      ...
      {days.length > 1 && (
        <div style={{
          ...
          <div style={{ maxWidth: '720px', margin: '0 auto' }}>
            ...
            <div style={{
              display: 'grid',
              gridTemplateColumns: `repeat(${Math.min(days.length, 2)}, 1fr)`,
              gap: '1.5rem',
            }}>
              {days.map((day, i) => {
                ...
                return (
                  <div key={i} style={{
                    ...
                    borderRadius: '12px',
                    textAlign: 'center',
                    background: isLast ? 'var(--cream)' : 'white',
                  }}>
                    <p style={{
                      fontFamily: 'Jost, sans-serif',
                    ...
                    <p style={{
                      fontFamily: 'Jost, sans-serif',
                      fontSize: '0.72rem', color: 'var(--mid-gray)',
          ...
          {wedding.venue_name && (
            <div style={{ padding: '1.5rem', background: 'white', borderRadius: '12px', border: '1px solid var(--light-gray)' }}>
              ...
              {wedding.venue_location && (
                <p style={{ fontFamily: 'Jost, sans-serif', fontSize: '0.78rem', color: 'var(--mid-gray)', marginTop: '0.3rem' }}>{wedding.venue_location}</p>
              )}
            </div>
          )}
